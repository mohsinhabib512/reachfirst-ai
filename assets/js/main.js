...
// Viewport-relative pointer depth for existing hero visuals. One RAF loop owns
// target capture, interpolation, and rendering across every page.
const cursorParallaxScenes = [...document.querySelectorAll([
  '.hero-artwork',
  '.services-page-map',
  '.industries-page-map',
  '.detail-visual',
...
if (cursorParallaxScenes.length) {
  const root = document.documentElement;
  const reducedMotion = matchMedia('(prefers-reduced-motion: reduce)');
  const finePointer = matchMedia('(hover: hover) and (pointer: fine)');
  const desktopViewport = matchMedia('(min-width: 768px)');
  ...
  const foregroundSelector = [
    '.hero-label',
    '.service-map-hub',
    '.industries-page-map-core',
    '.detail-visual-core',
    '.field-hero-board li.is-active',
    '.work-route-head strong',
    '.about-note-mark',
    '.case-proof-top strong',
    '.insights-brief-head svg',
    '.consultation-form-heading > span'
  ].join(',');
  const middleSelector = [
    '.service-map-node',
    '.industries-page-map-node',
    '.detail-chip',
    '.field-hero-board li:not(.is-active)',
    '.work-route li > span',
    '.about-note-path',
    '.case-proof-board li',
    '.insights-brief li > span'
  ].join(',');
  const backgroundSelector = '.hero-orbit-mask, .service-map-lines';

  cursorParallaxScenes.forEach((scene) => {
    scene.classList.add('cursor-parallax-scene');
    scene.querySelectorAll(foregroundSelector).forEach((layer) => layer.classList.add('cursor-parallax-foreground'));
    scene.querySelectorAll(middleSelector).forEach((layer) => layer.classList.add('cursor-parallax-middle'));
    scene.querySelectorAll(backgroundSelector).forEach((layer) => layer.classList.add('cursor-parallax-background'));
  });

  const render = () => {
    root.style.setProperty('--cursor-scene-x', `${(current.x * 6).toFixed(3)}px`);
    root.style.setProperty('--cursor-scene-y', `${(current.y * 5).toFixed(3)}px`);
    root.style.setProperty('--cursor-middle-x', `${(current.x * 5).toFixed(3)}px`);
    root.style.setProperty('--cursor-middle-y', `${(current.y * 4).toFixed(3)}px`);
    root.style.setProperty('--cursor-foreground-x', `${(current.x * 12).toFixed(3)}px`);
    root.style.setProperty('--cursor-foreground-y', `${(current.y * 10).toFixed(3)}px`);
    root.style.setProperty('--cursor-background-x', `${(current.x * -4).toFixed(3)}px`);
    root.style.setProperty('--cursor-background-y', `${(current.y * -3).toFixed(3)}px`);
    root.style.setProperty('--cursor-tilt-x', `${(current.y * -1.25).toFixed(4)}deg`);
    root.style.setProperty('--cursor-tilt-y', `${(current.x * 1.5).toFixed(4)}deg`);
  };
  ...
  const animate = (time) => {
    ...
    const frameScale = Math.min(Math.max((time - previousTime) / 16.667, 0.5), 2);
    previousTime = time;
    const interpolation = 1 - Math.pow(1 - smoothing, frameScale);
    current.x += (target.x - current.x) * interpolation;
    current.y += (target.y - current.y) * interpolation;
    if (Math.abs(target.x - current.x) < 0.0001) current.x = target.x;
    if (Math.abs(target.y - current.y) < 0.0001) current.y = target.y;
    render();
    frameId = requestAnimationFrame(animate);
  };

  const start = () => {
    if (frameId || !enabled) return;
  ...
  const onPointerMove = (event) => {
    ...
    target.x = Math.max(-1, Math.min(1, (event.clientX / innerWidth - 0.5) * 2));
    target.y = Math.max(-1, Math.min(1, (event.clientY / innerHeight - 0.5) * 2));
  };
  ...
  const syncPreference = () => {
    enabled = desktopViewport.matches && finePointer.matches && !reducedMotion.matches && !document.hidden;
    if (enabled) start();
    else {
  ...
  document.addEventListener('visibilitychange', syncPreference, { signal: abortController.signal });
